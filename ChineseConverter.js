/*******************************************
 * 簡繁切換程式
 * 程式設計根据(WFU Blog)：http://wayne-fu.blogspot.com/
 * 字庫來源(同文堂)：http://tongwen.openfoundry.org
 *******************************************/
(function(root, factory) {
  ...
  ChineseConverter.toTraditionalChinese = function(str) {
    if (!str) return str;
    return str.replace(/[^\x00-\xFF]/g, function(s) {
      var b = s_2_t[s];
      return b ? b : s;
  ...
  ChineseConverter.toTraditionalChineseForHtmlText = function(htmlNode) {
  ...
  ChineseConverter.toSimpliedChineseForHtmlText = function(htmlNode) {
    ...
    forEachTextNode(htmlNode, function(node) {
      node.nodeValue = node.nodeValue.replace(/[^\x00-\xFF]/g, function(s) {
        return ((s in t_2_s) ? t_2_s[s] : s);
      });
    });
  };

  var forEachTextNode = function(node, handle) {
    if (node.nodeType === 3) {
      handle(node);
    } else {
      for (node = node.firstChild; node; node = node.nextSibling) {
        forEachTextNode(node, handle);
      }
    }
  };
